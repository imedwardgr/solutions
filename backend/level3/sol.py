...
def getCommissions(days, totalPrice):
	us = int(totalPrice*0.3)
	insurance = int(us*0.5)
	assistance = days*100
	return {"insurance_fee":insurance, "assistance_fee":assistance, "drivy_fee":(us-insurance-assistance)}
...
with open("./data/input.json",'r') as file:
	...
	for row in range(len(data["rentals"])):
		...
		rental_price["rentals"].append({"id":(data["rentals"][row]["id"]), "price": int(getDaysPrice(days,ppDay) + distance*ppDistance), "commission":getCommissions(days,totalPrice)})
